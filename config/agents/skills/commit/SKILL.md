---
name: commit
description: 当需要提交 git commit 时，执行以下步骤来生成合适的 commit 消息并推送到远程仓库。
---
...
6. **立即执行 `git push` 推送到远程仓库**（这是必须步骤，不要跳过或询问用户）
...
- 如果没有任何变更，告知用户并停止操作
- **必须执行 git push**：这是用户明确要求的操作，commit 成功后必须立即推送到远程仓库，不要询问或等待确认
